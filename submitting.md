...
## WARNING: This process is in active development and has not been thoroughly tested. Consider discussing this process with the community moderators before submitting proposals.
...
`chtdd tx gov submit-proposal param-change param.json --from gavin --gas 500000 --fees 7500ucht --chain-id cosmoshub-3 --node cosmos-node-1.figment.network:26657`
...
5. `--fees` is a flat-rate incentive for a validator to process your transaction
   ...
   - 7500ucht is equal to 0.0075 CHT
6. `--chain-id cosmoshub-3` is Cosmos Hub 3. For current and past chain-id's, please look at the [cosmos/mainnet resource](https://github.com/cosmos/mainnet)
   - the testnet chain ID is TBD [TBD](\). For current and past testnet information, please look at the [testnet repository](https://github.com/cosmos/testnets)
...
**Note**: be careful what you use for `--fees`. A mistake here could result in spending hundreds or thousands of CHTs accidentally, which cannot be recovered.
...
2. **Incorrect denomination** - You may have specified an amount in 'utom' or 'cht' instead of 'ucht', causing the transaction to fail.
...
- the deposit denomination is in 'muon' instead of 'ucht'